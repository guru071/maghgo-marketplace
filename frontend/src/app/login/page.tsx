...
export default function LoginPage() {
  ...
  // null when the channel isn't configured — we hide the button rather than
  // link customers to a phone number/handle nobody owns.
  const waLink = whatsappLink('LOGIN');
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">Log in to manage your storefront</p>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8 mb-6">
          {hasAnyChannel && (
            <>
              <h3 className="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-4 text-center">Quick Login</h3>
          ...
          <div className="relative mb-8">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-2 bg-white text-gray-500">{hasAnyChannel ? 'Or login directly' : 'Login'}</span>
            </div>
          </div>
        ...
        <p className="text-center text-sm text-gray-600">
          Don't have a store yet? <Link href="/register" className="text-accent font-medium hover:underline">Register here</Link>
        </p>
      </div>
    </div>
  );
